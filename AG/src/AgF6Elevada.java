...
public class AgF6Elevada extends JFrame {

    public static Individuo[] pop;
    public static Individuo melhor;
    public static int pior;
    public static double[] fitness, fitnessNormalizado;
    ...
    public static void main(String[] args) {
        ...
        tamInd = 28;
        tamPop = 100;
        inc = 20; //0 se não for normalizado
        min = 10;
        elitismo = false;
    ...
    public static void normaliza() {
        ordenaDecPop();
        fitnessNormalizado = new double[pop.length];
        for (int i = 0; i < pop.length; i++) {
            fitnessNormalizado[i] = min + (inc * i);;
        }
    }
    ...
    public static double calculaFitness(double x, double y) {
        double f6 = 999.5 - (Math.pow((Math.sin(Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)))), 2) - 0.5) / Math.pow((1 + 0.001 * (Math.pow(x, 2) + Math.pow(y, 2))), 2);
        return f6;
    }
    ...
    public void paint(Graphics g) {
        ...
        funcao = AgF6Elevada.matrizF6(tamMat, maxXY);
        ...
        for (int i = 0; i < plano.length; i++) {
            for (int j = 0; j < plano[0].length; j++) {
                plano[i][j] = (float) (0.6666667 - ((funcao[i][j] - 999) * 0.6666667));
            }
        }
